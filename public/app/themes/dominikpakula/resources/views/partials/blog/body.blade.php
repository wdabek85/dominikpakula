...
<section class="mx-auto max-w-[1440px] px-4 lg:px-20 py-10 lg:py-16">
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
    ...
    <div class="lg:col-span-8 xl:col-span-9 min-w-0">
      ...
      {{-- Mobile TOC (details, open by default) — widoczny tylko gdy są nagłówki, JS to zrobi --}}
      ...
      {{-- Content (prose + editorial overrides) --}}
      ...
      {{-- Tagi --}}
      @if ($tags)
        <div class="mt-12 pt-6 border-t border-black/10 flex flex-wrap items-center gap-2">
          <span class="font-metro text-xs uppercase tracking-[3px] text-black/60 mr-2">Tagi</span>
          @foreach ($tags as $tag)
            <a
              href="{{ get_tag_link($tag->term_id) }}"
              class="font-poppins text-xs text-black border border-black/20 rounded-sm px-3 py-1 hover:bg-black/5 transition-colors"
            >
              #{{ $tag->name }}
            </a>
          @endforeach
      ...
      {{-- Mobile share (powtórzony niżej, sidebarowy nie jest widoczny na mobile) --}}
